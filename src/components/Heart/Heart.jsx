...
function Heart({ bookId }) {
    ...
    useEffect(() => {
        if (bookId) {
            const saved = localStorage.getItem(`book-${bookId}-liked`);
    ...
    const handleChange = (e) => {
        ...
        setIsLiked(liked);
        if (bookId) {
            localStorage.setItem(`book-${bookId}-liked`, liked.toString());

            // Émet un événement personnalisé que Card.jsx peut écouter
            window.dispatchEvent(new CustomEvent('heart-changed', {
                detail: { bookId, liked }
            }));
        }
    };
